...
@description('Deploy the active-active VPN Gateway. Set to false to deploy Azure Firewall only (no site-to-site VPN).')
...
resource pipVpnGw1 'Microsoft.Network/publicIPAddresses@2023-06-01' = if (deployVpnGateway) {
  name: pipVpnGw1Name
  location: location
  tags: tags
  sku: { name: 'Standard', tier: 'Regional' }
...
resource pipVpnGw2 'Microsoft.Network/publicIPAddresses@2023-06-01' = if (deployVpnGateway) {
  name: pipVpnGw2Name
  location: location
  tags: tags
  sku: { name: 'Standard', tier: 'Regional' }
...
// ---------------------------------------------------------------------------
resource vpnGateway 'Microsoft.Network/virtualNetworkGateways@2023-06-01' = if (deployVpnGateway) {
  name: vpnGwName
  location: location
...
output vpnGwPip1           string = deployVpnGateway ? pipVpnGw1.properties.ipAddress : ''
output vpnGwPip2           string = deployVpnGateway ? pipVpnGw2.properties.ipAddress : ''
